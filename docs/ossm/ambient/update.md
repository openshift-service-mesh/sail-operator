...
For general information about update strategies (InPlace and RevisionBased), see [Update Strategy](../../update-strategy/update-strategy.adoc).

For information about installing Istio in ambient mode, see [Istio Ambient Mode](../../common/istio-ambient-mode.adoc).
...
## 1.2. About Istio update process
...
### 1.2.1. About Istio control plane update strategies

The Sail Operator supports two update strategies: InPlace and RevisionBased. For detailed information about these strategies and their general behavior, see [Update Strategy](../../update-strategy/update-strategy.adoc).
...
For general information about the InPlace strategy, see the [InPlace](../../update-strategy/update-strategy.adoc#inplace) section in the Update Strategy documentation.
...
### 1.3.1. Selecting InPlace strategy
...
### 1.3.2. Updating Istio control plane with InPlace strategy
...
After updating the Istio control plane, proceed to update IstioCNI, ZTunnel, verify ambient workloads, and optionally update Waypoint Proxies. Refer to section 1.5 "Common Update Procedures for Ambient Components" for detailed instructions on updating these components.

### 1.3.3. Recommendations for InPlace strategy in Ambient Mode
...
## 1.4. About RevisionBased strategy
...
For general information about the RevisionBased strategy, see the [RevisionBased](../../update-strategy/update-strategy.adoc#revisionbased) section in the Update Strategy documentation.
...
### 1.4.1. Selecting RevisionBased strategy
...
### 1.4.2. Updating Istio control plane with RevisionBased strategy
...
```bash
$ kubectl exec "$(oc get pod -l app=ratings -n bookinfo -o jsonpath='{.items[0].metadata.name}')" -c ratings -n bookinfo -- curl -sS productpage:9080/productpage | grep -o "<title>.*</title>"
...
If you have deployed waypoint proxies, verify them after the upgrade. Refer to section 1.5.4 "Updating Waypoint Proxies (If Deployed)" for detailed instructions.

### 1.4.3. Rollback Procedure
...
## 1.5. Common Update Procedures for Ambient Components
...
### 1.5.3. Verifying Ambient Workloads
...
### 1.5.4. Updating Waypoint Proxies (If Deployed)

If you have deployed waypoint proxies in your ambient mesh, they should be verified after the control plane upgrade:
...
## 1.6. About the Istio CNI update process
...
For detailed procedures on updating IstioCNI, see section 1.5.1 "Updating IstioCNI".

---

## 1.7. Special Considerations for Ambient Mode Upgrades

### 1.7.1. ZTunnel DaemonSet Updates
...
### 1.7.3. Control Plane and Data Plane Version Skew
...
### 1.7.4. Waypoint Proxy Compatibility
...
### 1.7.5. Impact on Existing Ambient Workloads
...
### 1.7.6. Discovery Selectors Impact
...
* **Istio Ambient Mode Installation:** [../../common/istio-ambient-mode.adoc](../../common/istio-ambient-mode.adoc)
* **Istio Ambient Waypoint Proxy Guide:** [../../common/istio-ambient-waypoint.adoc](../../common/istio-ambient-waypoint.adoc)
* **General Update Strategy Documentation:** [../../update-strategy/update-strategy.adoc](../../update-strategy/update-strategy.adoc)
