...
oc --kubeconfig "${kubeconfig}" -n zero-trust-workload-identity-manager \
...
# check Spire CSI driver
  ...
  echo "All SPIRE components are healthy on ${kubeconfig}"
done
----
...
_⚠️ This verification-only step grants the `privileged` SCC and runs the
`spiffe-mtls-validator` pods as root (`runAsUser: 0`). This is required here
because the pods mount the SPIRE Agent socket (`spire-agent.sock`) directly and
read it outside of an Istio sidecar, and the socket is owned by root. Do not use
this pattern for production workloads — the Istio integration sections above
consume the same socket through the CSI driver without privileged access. Run
this procedure only on disposable/test clusters, and delete the
`verify-spiffe-federation` namespace and the two `add-scc-to-user` grants once
verification is complete._
...
# create namespace
oc create namespace $NAMESPACE --kubeconfig="${CLUSTER_A_KUBECONFIG}"
...
export NAMESPACE=verify-spiffe-federation
# create namespace
oc create namespace $NAMESPACE --kubeconfig="${CLUSTER_B_KUBECONFIG}"
# allow privileged SCC because we going to mount spife-agent.sock directly into the pod, this is done for verification needs only
...
oc wait --for=condition=Ready pod -l app=spiffe-mtls-validator -n $NAMESPACE --kubeconfig="${CLUSTER_A_KUBECONFIG}" --timeout=120s
# Wait for client pod on Cluster B
oc wait --for=condition=Ready pod -l app=spiffe-mtls-client -n $NAMESPACE --kubeconfig="${CLUSTER_B_KUBECONFIG}" --timeout=120s
# Check the logs, if ok expect to see something like "[body: success] [status: 200 OK]"
if oc logs -l app=spiffe-mtls-client \
  -n $NAMESPACE --kubeconfig="${CLUSTER_B_KUBECONFIG}" --tail=20 \
  | grep -q "\[body: success\] \[status: 200 OK\]"; then
...
printf '%s\n' "${SLEEP_SERVICE_URI}" "${HELLO_WORLD_SERVICE_URI}"
----
...
----
# sleep service
URI:spiffe://<cluster-a-trust-domain>/ns/<namespace>/sa/<service-account>
# helloworld service
URI:spiffe://<cluster-b-trust-domain>/ns/<namespace>/sa/<service-account>
----
...
[source,bash]
----
# allow istio ingress pod to run with anyuid on Cluster A
oc adm policy add-scc-to-user anyuid -z istio-gateway -n "${OSSM_NS}" --kubeconfig=$CLUSTER_A_KUBECONFIG
# install the istio gateway helm chart
helm install istio-gateway -n "${OSSM_NS}" \
  istio/gateway --set-json \
...
curl -sS -o /dev/null -w "%{http_code}\n" -H "Host: helloworld" http://$INGRESS_IP/hello
----
