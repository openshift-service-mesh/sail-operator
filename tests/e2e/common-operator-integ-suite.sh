...
if [ -z "${GOMEMLIMIT:-}" ]; then
  ...
  if [ -f /sys/fs/cgroup/memory.max ]; then
    ...
    echo "DEBUG: No cgroups memory limit file found, using default 512MiB"
    # Default to 512MiB if we can't detect the limit - leave room for child processes (kubectl, oc, etc.)
    GOMEMLIMIT="512MiB"
  fi
